  ...
  return (
    <div className="relative flex max-w-[8rem] items-center">
      <button
        ...
        onClick={() => onChange(-1)} // dec button should decrese
      >
        <svg
      ...
      <input
        ...
        required={false}
        value={timeSeconds}
        onChange={(e) => onChange(Number(e.target.value))} // change to current value in input field
      />
      <button
        type="button"
        id="increment-button"
        ...
        onClick={() => onChange(timeSeconds + 1)}
      >
        <svg
          className="h-3 w-3 text-gray-900 dark:text-white"
          aria-hidden="true"
